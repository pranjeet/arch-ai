...
export function ShapePanel() {
  ...
  function handleDragStart(event: React.DragEvent, shape: NodeShape) {
    ...
    event.dataTransfer.setData("application/arch-shape", payload)
    event.dataTransfer.effectAllowed = "copy"

    // Replace the default browser drag image with a transparent pixel
    const arch = document.createElement("div")
    arch.style.cssText = "position:fixed;top:-9999px;left:-9999px;width:1px;height:1px;"
    document.body.appendChild(arch)
    event.dataTransfer.setDragImage(arch, 0, 0)
    setTimeout(() => document.body.removeChild(arch), 0)
